...
- [Объяснение названий колонок](#объяснение-названий-колонок)
...
## От 03.04.2026 (обновлённая инструкция)
1) Скачиваем другой клиент - [Throne](https://github.com/throneproj/Throne/releases/download/1.1.1/Throne-1.1.1-windows64-installer.exe), **Nekoray** убираем из автозапуска, и вообще, можете его удалить.

2) Устанавливаем в удобную папку (желательно, на всякий случай, чтобы в пути не было кириллицы или спец символов).
3) После запуска программы попросите меня скинуть QR-код (или найдите последний мною скинутый QR-код, можете в ЛС со мной в ТГ вбить в поиск "vless", по идее найдёте) и, не закрывая окно с QR-кодом, заходим в Throne и нажимаем **Программа** > **Сканировать QR-код.** Если всё прошло хорошо, то в вашей программе появится новое соединение! Нажимаем по нему **ПКМ** > **Запустить.**
Также, вместо QR-кода, можно скопировать текст, который я скинул вместе с QR-кодом, и вместо **Сканировать QR-код** нажать **Добавить профиль из буфера обмена**.

4) В интерфейсе программы заходим в **Настройки** > **Настройки маршрутизации** > **DNS**.

[![Skrinsot-20260403-170528.png](https://i.postimg.cc/Prn5jPJc/Skrinsot-20260403-170528.png)](https://postimg.cc/XXxbKNwc)

Ставим на "Удалённый DNS"

> https://dns.google/dns-query 

а на DNS  для прямых запросов 

>77.88.8.8

5) Далее заходим на следующую вкладку сверху **Маршрут** , выбираем **Default** , нажимаем снизу **Редактировать** . Ставим **Outbound по умолчанию** : **direct** .
6) Выбираем в этой же менюшке вкладку **Дополнительно** и нажимаем **Импорт JSON** . Туда вставляем этот конфиг из [этого файла](https://github.com/Shurupiii/Shurupi-proxy/blob/main/config.md) .
7) Чтобы добавить какие-либо процессы, домены и т.д. можно воспользоваться этими списками, туда пишете условно 
...
[![Skrinsot-20260403-170430.png](https://i.postimg.cc/nVjXxyk0/Skrinsot-20260403-170430.png)](https://postimg.cc/QVD8knvT)

**process_name** - имя процесса в диспетчере задач.

**domain** - конкретный домен. Например: домен abc.example.domain не будет проксироваться (или идти напрямую, смотря в какую колонку вы его вписали), но ИМЕННО example.domain - будет

**suffix** - все домены, оканчивающиеся на это. Например: **chatgpt.com** будет проксироваться (или идти напрямую, но я не буду это писать 100 раз), и домены **test.chatgpt.com**, **123.test.ochen.dlinniy.domen.chatgpt.com** будет проксироваться

**keyword** - все домены, содержащие указанное слово. Например: **youtube.com**, **youtube.ru** (но в JSON конфиге, который я прикрепил, все ru домены идут напрямую), **youtubeshurup.org**

8) **Настройки > Настройки режима TUN** ставите **Stack: mixed** и выключаете **Строгий маршрут** . Все процессы, записанные в колонку прокси, пойдут через TUN режим, если его включить. 

Например: для того, чтобы поиграть на заблокированном сервере **mc.hypixel.net** , вам нужно включить TUN режим, добавить процесс Майнкрафта в колонку прокси: process_name:javaw.exe . Если не работает, то добавить в колонку прокси ещё и домен сервера: suffix:mc.hypixel.net .

Вообще, если вы что-то не поняли из моих объяснений (что более чем нормально), то можете ознакомиться со статьёй про маршрутизацию в Throne, написанной с уважением к её читателю: https://blancvpn.care/ru/help/throne-split-tunneling . 
